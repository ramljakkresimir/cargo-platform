...
- `backend/src/data-source.ts` — loads `.env` via `dotenv` (added as an explicit `backend/package.json` dependency rather than relying on it being hoisted transitively through `@nestjs/config`, given this project's documented history of workspace-hoisting bugs — see "Resolved: npm workspaces hoisting conflict" above).
