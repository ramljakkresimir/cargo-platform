...
describe('CargoPostsService', () => {
  ...
  beforeEach(() => {
    ...
    citiesService = {
      findById: jest.fn().mockResolvedValue({ id: 'city-a', name: 'Sarajevo', country: 'BA' }),
    };
    service = new CargoPostsService(repo, citiesService);
  });
  ...
  describe('update — owner status transitions (M1)', () => {
    ...
    it('allows closed -> active when the loading date is still current', async () => {
      const post = makePost({ status: PostStatus.CLOSED, loadingDate: localDateString(0) });
      repo.findOne.mockResolvedValue(post);
    ...
    it('rejects reactivating a closed post whose loading date has passed', async () => {
      const post = makePost({ status: PostStatus.CLOSED, loadingDate: localDateString(-1) });
      repo.findOne.mockResolvedValue(post);
