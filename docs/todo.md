...
- [x] Redesigned vehicle/cargo detail pages (Session 26) — shared `components/detail/` kit driven by one `DetailData` object per page (mirrors the search pages' `ResultCardData` pattern), route-headline header with fact tiles, sticky map+contact/rating layout, horizontal/vertical cities stepper, masked-contact reveal, full responsive spec (desktop/tablet/mobile with a sticky mobile action bar); cargo posts gained real driving-route geometry (`routeGeoJson`, no cities-on-route matching — that stays vehicle-only)
- [x] Redesigned home page "how it works" section (Session 27) — illustrated 3-column desktop mockups with dotted connectors sized via `calc()` to span exact card-to-card edges, plus a dedicated vertical-timeline design for tablet/mobile switching at the project's existing 1024px breakpoint; also fixed a pre-existing hero-heading left-edge misalignment against the rest of the page
